 ...
 */
static void cpufreq_notify_utilization(struct cpufreq_policy *policy,	unsigned int util)
{
	if (policy) {
		policy->util = util;
	}
...
static inline /*cputime64_t*/ u64 get_cpu_idle_time(unsigned int cpu, /*cputime64_t*/ u64 *wall)
{
...
static void update_sampling_rate(unsigned int new_rate)
{
	...
	dbs_tuners_ins.sampling_rate = new_rate	= max(new_rate, min_sampling_rate);
